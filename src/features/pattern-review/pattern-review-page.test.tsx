...
describe('PatternReviewPage', () => {
  it('keeps the first detected level instead of replacing it with a later higher peak', async () => {
    ...
    renderPage()
    await screen.findByText('Свеча распознавания: 20 сент. 2025 г. UTC')
    expect(screen.queryByText('Свеча распознавания: Later date UTC')).not.toBeInTheDocument()
  ...
  it('shows the full continuation by default and still supports candle replay', async () => {
    ...
    expect(screen.getByText('Порядок событий внутри свечи неоднозначен')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Скрыть продолжение' }))
    expect(screen.getByTestId('pattern-chart')).toHaveTextContent('candles:1')
    await user.click(screen.getByRole('button', { name: '+1 свеча' }))
    expect(screen.getByTestId('pattern-chart')).toHaveTextContent('candles:2')
    expect(screen.queryByText('Порядок событий внутри свечи неоднозначен')).not.toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Далее' }))
  ...
  it('shows an honest empty result, not an endless search', async () => {
    ...
    renderPage()
    expect(await screen.findByText('0 кандидатов')).toBeInTheDocument()
